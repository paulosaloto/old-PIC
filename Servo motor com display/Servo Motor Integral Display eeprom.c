...
void def_hora (int destino/*0 para memoria e 1 para rtc*/)
{
   ...
   switch(tecla){
      ...
      case(12):
         ...
         else write_ext_eeprom(posicao,hora);
         tecla=0;
         return;
      default:
         hora_menor=tecla;
...
void def_min (int&min, int h/*3 para segundos e 2 para minutos*/, int destino/*0 para memoria e 1 para rtc*/)
{
   ...
   switch(tecla){
      ...
      case(12):
         ...
         else (write_ext_eeprom(posicao,min));
         tecla=0;
         return;
      default:
         min_menor=tecla;
...
////////////////////////////////////////////////////////////////////////////////
   printf(lcd_putc,"\fInsira hora atual:");
   while(horario!=3){
      varredura();
      write_horaminseg(1);
   }
   printf(lcd_putc,"\fInsira data:");
...
int ordem;
      while(quantidade!=ordem+1)
      {
         ...
         switch(igualdade)
         {
          ...
          //         | 20º    yy:yy:yy|
            case(0):
            if(mes==read_ext_eeprom(1+6*ordem))   bit_set(igualdade,4);//0001 0000
            else  break;
            
            case(0x10):
            if(dia==read_ext_eeprom(2+6*ordem))   bit_set(igualdade,3);//0001 1000
            else  break;
            
            case(0x18):
            if(hora==read_ext_eeprom(3+6*ordem))   bit_set(igualdade,2);//0001 1100
            else  break;
            
            case(0x1c):
            if(minutos==read_ext_eeprom(4+6*ordem))   bit_set(igualdade,1);//0001 1110
            else   break;
            
            case(0x1e):
            if(segundos==read_ext_eeprom(5+6*ordem))   bit_set(igualdade,0);//0001 1111
            else  break;
            
            case(0x1f):
            angulo_pedido=read_ext_eeprom(6+6*ordem);
            ordem=ordem+1;            
