...
int hora2=0, angulo2=0, minutos2=0, segundos2=0;
...
void def_dia ()//Entra primeiro algarismo, entra #. Ou entra primeiro, entra segundo, entra #.
{
   ...
   switch(tecla){
      ...
      case(12): //Tem que funcionar para quando tem 1 só algarismo e para quando tem 2 algarismos
         ...
         if((dia_maior==0)&&(dia_menor==0))
            dia = 1;
         data=3;write_RTC(5,dia+resto);
         tecla=0;
         return;
      default:   
         dia_menor=tecla;
   ...
   if(dia_maior>0x03)
      ...
      if ( ((mes==0x01)||(mes==0x03)||(mes==0x05)||(mes==0x07)||(mes==0x08)||(mes==0x10)||(mes==0x12)) && (dia_maior*16+dia_menor>0x31)){
         dia = dia_menor;
         tecla=0;
         return;
      }
      if ( ((mes==0x04)||(mes==0x06)||(mes==0x09)||(mes==0x11)) && (dia_maior*16+dia_menor>0x30)){
         dia = dia_menor;
         tecla=0;
         return;
      }
      if ( (mes==0x02) && (bit_test(resto,7)==0) && (bit_test(resto,6)==0) && (dia_maior*16+dia_menor>0x29)){
         dia = dia_menor;
         tecla=0;
         return;
      }
      if ( (mes==0x02) && ((bit_test(resto,7)!=0)||(bit_test(resto,6)!=0)) && (dia_maior*16+dia_menor>0x28)){
         dia = dia_menor;
         tecla=0;
         return;
      }
...
void def_mes ()//Entra primeiro algarismo, entra #. Ou entra primeiro, entra segundo, entra #.
{
...
void Define_referencia(int faixa)
{
   ...
   switch(faixa)
   {
      ...
      case(3):
      valor_aux = (-0.0356)*angulo_pedido*angulo_pedido + 16.022*angulo_pedido - 731.71; //Esse é double, então o número é "quebrado"
      break;
...
void funcao_pwm()
{
   ...
   if(sentido_ant!=sentido)//quando o ponteiro passa do ponto pedido, a ação integral zera(tem que ver se ta certo)
      ...
      if (erro<341) integral=(100*integral+erro)*0.01;//a ação integral só ativa para um erro menor, afim de que ela não estoure.
   
   controle = (erro*3+integral);//o controle é feito pela ação proporcional com Kp=4 mais a ação integral 
