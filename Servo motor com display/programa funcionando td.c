...
//Variáveis globais//
int1 sentido_ant=1;
unsigned int angulo_pedido, angulo_pedido_anterior;
...
int hora=0, angulo=0, minutos=0, segundos=0, dia =0, mes =0, ano =0, data =0, resto=0, horario=0, aux_timer=0, posicao=0, quantidade=0;
int igualdade=0;
...
void def_angulo ()
{
   switch(tecla){
      ...
      case(11):
         ...
         break;
      case(12):
         write_ext_eeprom(posicao*6+6,angulo);
         posicao=posicao+1;
         tecla=0;
         angulo=0;
         numero=5;
...
////////////////////////////////////////////////////////////////////////////////         
      ...
      {
         horario=0;
         printf(lcd_putc,"\fHorario Angulo %d:",posicao+1);
         while(horario!=3)
         {
            varredura();
            write_horaminseg(0);
         ...
         data=0;
         printf(lcd_putc,"\fData Angulo %d:",posicao+1);
         while(data!=3) //Também vai aumentando até ter ano, mes e dia.
         {
            varredura();
            write_data(0);
         }
                  printf(lcd_putc,"\fValor Angulo %d:",posicao+1);
...
hora2=read_ext_eeprom(3+6);
minutos2=read_ext_eeprom(4+6);
segundos2=read_ext_eeprom(596);
dia2=read_ext_eeprom(2+6);
mes2=read_ext_eeprom(1+6);
angulo2=read_ext_eeprom(6+6);

////////////////////////////////////////////////////////////////////////////////
////      AÇÃO DE LEITURA DO RTC E COMPARAÇÃO COM OS DADOS DA MEMÓRIA       ////
////////////////////////////////////////////////////////////////////////////////
//função de captura dos dados do RTC e exibição no display
//verifica se o horário mostrado no display é igual ao horário pedido nos ângulos
//cada variável a comparar corresponde a 1 bit da variável 'igualdade', onde bit0=segundos, bit1=minutos etc.
//o switch é feito para que a comparação seja feita apenas com os valores que apresentam diferença.

posicao=0;
      while(quantidade!=posicao)
      {
         enable_interrupts(INT_timer0);
         if (aux_timer==0) show_time();
         switch(igualdade)
         {
          //display: |ang.n   xx:xx:xx|
          //         | 20º    yy:yy:yy|
            case(0):
            if(mes==read_ext_eeprom(1+6*posicao))   bit_set(igualdade,4);//0001 0000
            else  break;
            
            case(0x10):
            if(dia==read_ext_eeprom(2+6*posicao))   bit_set(igualdade,3);//0001 1000
            else  break;
            
            case(0x18):
            if(hora==read_ext_eeprom(3+6*posicao))   bit_set(igualdade,2);//0001 1100
            else  break;
            
            case(0x1c):
            if(minutos==read_ext_eeprom(4+6*posicao))   bit_set(igualdade,1);//0001 1110
            else   break;
            
            case(0x1e):
            if(segundos==read_ext_eeprom(5+6*posicao))   bit_set(igualdade,0);//0001 1111
            else  break;
            
            case(0x1f):
            angulo_pedido=read_ext_eeprom(6+6*posicao);
            posicao=posicao+1; 
            igualdade=0;
